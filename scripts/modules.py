...
def count_tags(data):
    ...
    count_df = pd.Series(
        data.loc[:, question_tags_featured].squeeze().values.reshape(-1)
    ).value_counts()
    ct_df = pd.DataFrame(
        {
            "Tag": count_df.index,
...
    En entrée nous avons une chaîne brute et en sortie une seule chaîne. Nous utilisons les stopwords et lemmatisation.
    ...
    lemmatize_meaningful_words = [
        lemmatizer.lemmatize(w, get_wordnet_pos(w)) for w in meaningful_words
    ]
...
def data_preprocessing(data):
    ...
    bodytitle_words_features = vectorizer.fit_transform(
        bodytitle_words_clean.body_title_words
    )
    print("Vectorisation de Body_Title effectuée")
    print(
        "Taille suite à vectorisation de Body_Title sur training {}".format(
            bodytitle_words_features.shape
        )
    )
...
def data_preprocessing_test(data, vectorizer):
    ...
    bodytitle_words_features_test = vectorizer.transform(
        bodytitle_words_clean.body_title_words
    )
    print("Vectorisation de Body_title effectuée")
    print(
        "Taille suite à vectorisation de Body_Title sur test {}".format(
            bodytitle_words_features_test.shape
        )
    )
    print("Fin du data preprocessing")
...
def ml_create_target_df(selected_tags, data_set):
    ...
    question_tags_featured = ["Tag_1", "Tag_2", "Tag_3", "Tag_4", "Tag_5"]
    print(
        " La dataframe cible contient {} tags...Traitement en cours".format(
            selected_tags.shape[0]
        )
    )
    ...
    for i, tag in zip(range(temp_array.shape[1]), selected_tags.values):
        temp_array[:, i] = np.sum(
            data_set.loc[:, question_tags_featured] == tag, axis=1
        )
...
def mean_local_recall(truth_pred_comparison_df, unique_tags_serie):
    ...
    truth_pred_comparison_df["Local_Recall"] = truth_pred_comparison_df.apply(
        local_recall, axis=1
    )
...
def other_scores(truth_pred_comparison_df, unique_tags_serie):
    ...
    y_pred_splitted = truth_pred_comparison_df.Predicted_Tags.apply(
        lambda x: x.split("/")
    )
    ...
    f1_score = metrics.f1_score(
        binarizer.transform(y_true_splitted),
