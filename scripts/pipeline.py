...
for file in allFiles:
    ...
    try:
        ...
        print(
            "Merci de vérifier la présence de {} dans le répertoire 'data'".format(file)
        )
...
top_tags = count_tags_df[
    count_tags_df["Pourcentage (%)"].cumsum(axis=0) < threshold_percentage
]
...
print(
    "Sélection de {} Tags dans notre jeu d'entrainement".format(selected_tags.shape[0])
)
...
data_train["New_tags"] = data_train[question_tags_featured].apply(
    lambda x: "/".join(x.dropna()), axis=1
)
data_test["New_tags"] = data_test[question_tags_featured].apply(
    lambda x: "/".join(x.dropna()), axis=1
)
